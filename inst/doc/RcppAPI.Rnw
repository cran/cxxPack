...
from {\tt configure.in} and {\tt configure.win}. It is recommended that you
keep the line that writes {\tt Rcpp-version.txt} so that it is clear what
version of \Rcpp\ was used to build your package. Remember to run {\tt autoconf}
