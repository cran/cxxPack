...
{\tt Rcpp.cpp} and {\tt Rcpp.hpp}. They are located in {\tt inst/lib}
before installation, and they are placed into the {\tt lib} directory
after installation (not to be confused with {\tt libs}, where the
package shared library is placed).

During the installation process the \Rcpp\ (static) 
library {\tt libRcpp.a} is built in
{\tt inst/lib}, and the object files created in {\tt src} are linked against
this library in order to create the package shared library
{\tt RcppTemplate.so} (DLL under Windows). The static library
...
libraries by placing them into {\tt inst/lib} and modifying the
scripts so that the appropriate compiler flags are inserted (see
{\tt configure.in}, {\tt configure.win}, 
{\tt Makefile.win}, for example, and note that {\tt autoconf} must be
run after modifying {\tt configure.in}). 

It is also possible to link
against external libraries, but in this case some code should be inserted
(into {\tt configure.in}, for example) that checks for the existence of
these libraries. For example, the {\tt RQuantLib} package uses \Rcpp\ and
needs to link against the {\tt QuantLib} and {\tt Boost} class libraries. Its
{\tt configure.in} file does the necessary checking.\footnote{If you want to
