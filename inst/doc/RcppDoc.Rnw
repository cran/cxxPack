...
%\VignetteIndexEntry{R/C++ Object Mapping Library and Package Template}
...
\author{Dominick Samperi\footnote{This is an updated version of the
    ...
    R/C++ Interface Classes Version 5.0}, September 20, 2006. I am
...
\markright{\textsc{Copyright (C) 2005-2009 Dominick Samperi}}
...
There are several interface packages available that enable \R\ to
communicate with other language systems including Java, Perl and
Python. An important goal of these packages is to expand the \R\
user community by making \R\ multi-lingual. The goal of Rcpp is
somewhat different. Here we want to give C++ the ability to
speak the languge of \R: vectors, matrices, data frames, time
series, etc. with the goal of faciliting communication between the
two languages, and more importantly, improving the performance of
\R\ packages.
...
the model object in the address space of the \C++ module. The
...
it is possible to work directly with \R's address space, and there
are classes that help you to do this, but they are intended to be used
in situations where one has to work with huge data sets where the
copy overhead is not acceptable. These facilities should not be used
routinely because this is inconsistent with the pass-by-value
semantics of \R.
...
run when the package is loaded.\footnote{Use the {\tt search()} command
  to view the package search list (and order), and use 
  {\tt ls(package:RcppTemplate)} to view the objects in this package
  including namespace exports. Use {\tt str(obj)} to view the structure
...
to override the default behavior of functions like {\tt print()} that are
automatically called when the name of a variable is used in certain
contexts. The default behavior of {\tt print()} is to print everything
...
from a ``clean'' state. Under UNIX the {\tt cleanup} script can be used
for this purpose; under Windows use {\tt sh cleanup.win}.}
...
packages this one depends on and download and install
those packages automatically.
...
The {\tt inst} directory contains {\tt LICENSE-Rcpp.txt} and
{\tt LICENSE.txt}. Both files will be copied to the root directory of
the installed package. If desired license information can be inserted into
the latter file (license information must appear in the
{\tt DESCRIPTION} file).
