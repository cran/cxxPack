...
example. The logic should be easy to follow even though we do not
discuss syntax in detail until later. We also explain how vignettes
...
starting point when creating a new package.

Section~\ref{sec.examples} presents a number of examples using most
of the classes from {\bf cxxPack}, again without getting into a lot of
...
the package library and sets flags {\tt compile} and {\tt quiet} that
are used to control the behavior of {\tt loadcppchunk()}). When
{\tt compile} is {\tt FALSE} the compilation step is suppressed, and
when {\tt quiet} is {\tt TRUE} output is less verbose (no effect
under Windows though). See the man page for {\tt loadcppchunk()} for
...
\begin{Verbatim}[formatcom=\color{cyan}]
 \srcinclude[red]{testHello}
\end{Verbatim}

This basically includes {\tt cxxPack/src/testHello.cpp} in verbatim
...
the {\tt lib} chunk. Since {\tt testHello.cpp} is in the package {\tt src}
subdirectory the compiled function is stored in the package shared library,
and the {\tt .Call()} here runs that function, producing the expected
result.

\subsection{Dot product}
\label{sec.dotproduct}

Next we consider the problem of calling a \C++ function whose source
file is not in the package {\tt src} subdirectory. The
file {\tt testDotProduct.cpp} is located in 
...
This function simply computes the dot product of two input vectors.
The {\tt RcppExport}
macro does whatever is necessary to ensure that this function is
exposed by the shared library that it is written to (OS-dependent). 
The logic should be easy to follow if not the detailed syntax. For
details on the syntax see Section~\ref{sec.nutshell}.
...
\begin{Verbatim}[formatcom=\color{cyan}]
 ...
 <<lib>>
 loadcppchunk('testDotproduct',compile=compile,quiet=quiet)
 x <- 1:5
 y <- 1:5
 sum(x*y)
...
required. When {\tt quiet} is {\tt TRUE} compilation output is less
verbose (under Linux).
...
Assume that the source archive for {\bf cxxPack} has been unpacked so
the root of the corresponding package hierarchy is in the current
working directory. To build a package source archive use:
\begin{verbatim}
$ R CMD build cxxPack
\end{verbatim}
This will generate {\tt cxxPack\_7.0.tar.gz} (and will overwrite the
original source archive if it was not moved to a different location).
This command processes the vignette so the corresponding PDF file is
contained in this archive.

The vignette processing step can be skipped using:
\begin{verbatim}
$ R CMD build --no-vignettes cxxPack
\end{verbatim}
If vignette processing was not previously done the result will be
an source archive {\tt cxxPack\_7.0.tar.gz} that is missing vignette
PDF files. To distinguish this case from the last one we will call this
an \emph{unprocessed} source archive. Source archives at CRAN are normally
of the processed variety (they include vignette PDF files).

The {\tt --no-vignettes} option is useful for building a source archive
...
\begin{verbatim}
$ tar -xvzf cxxPack_7.0.tar.gz
$ Rcmd build --binary --no-vignettes cxxPack
\end{verbatim}
The output file is {\tt cxxPack\_7.0.zip}.
...
other scratch files are created that are only needed for this processing.
These files should not go into an archive intended for distribution
(either {\tt tar.gz} or {\tt zip}).
Under Linux this is taken care of automatically because the
script file {\tt cxxPack/cleanup} is run before the archive is
created. Unfortunately, the current version of R does not run
{\tt cxxPack/cleanup.win} before creating the archive.

Thus to create an archive
under Windows with vignette processing use something like:
\begin{verbatim}
$ tar -xvzf cxxPack_7.0.tar.gz
$ Rcmd build --binary cxxPack
$ cd cxxPack; sh cleanup.win; cd ..
$ Rcmd build --binary --no-vignettes cxxPack
\end{verbatim}
The first {\tt Rcmd build} here will process the vignette, but it will
create a large {\tt zip} file because {\tt cleanup.win} is not called.
So we call it explicitly and use {\tt Rcmd build} again with the
{\tt --no-vignettes} option. The final {\tt cxxPack\_7.0.zip} will not
be too large.

Before a final INSTALL or submission to CRAN a package
is normally checked using:\footnote{If the source archive is unprocessed
  this command will try to process the vignette. This will not work
  with \S++ (see the technical notes). Thus only use {\tt check} on
  processed source archives that include vignette PDF files.}
\begin{verbatim}
...
\end{verbatim}
and the vignette can be viewed by staring R and using:
\begin{verbatim}
...
(after its corresponding package has been installed) as follows:
\begin{verbatim}
$ cd cxxPack/inst/doc
$ make
\end{verbatim}

The {\tt Makefile} contains the following:
\begin{verbatim}
target:
    R CMD Sweave cxxPackGuide.Rnw
    pdflatex cxxPackGuide.tex
    bibtex cxxPackGuide
    pdflatex cxxPackGuide.tex
    pdflatex cxxPackGuide.tex

clean:
    rm -f <temp files...see Makefile>
\end{verbatim}
First {\bf Sweave} is used to transform {\tt cxxPackGuide.Rnw} into
{\tt cxxPackGuide.tex}. Then the standard \TeX\ tools are used to
process this \TeX\ file in the usual way (running {\tt pdflatex} twice
at the end to get the cross references right). Another way to speed
the development process is to use the {\tt compile} option of
{\tt loadcppchunk()}---see Section~\ref{sec.dotproduct}.

With this method the development cycle switches from
edit {\tt Rnw} file, {\tt build}, {\tt INSTALL}, run R,
run {\tt vignette()} to: edit {\tt Rnw file}, {\tt make},
view PDF file.

As we noted above, it is important that vignette shared libraries and
other scratch files not go into a package archive to be distributed.
When it is time to create a package archive without a lot of
large files that are no longer needed use:
\begin{verbatim}
$ cd cxxPack/inst/doc
$ make
$ make clean
...
loadcppchunk('testHighFreqSeries',compile=compile,quiet=quiet)
...
loadcppchunk('testPaymentSchedule',compile=compile,quiet=quiet)
...
<<lib>>
  loadcppchunk('testFFT',compile=compile,quiet=quiet)
foo <- .Call('testFFT')
...
<<lib>>
  loadcppchunk('testRootFinder',compile=compile,quiet=quiet)
...
<<BankAccount.xptr.R>>=
<<lib>>
  loadcppchunk('testBankAccount',compile=compile,quiet=quiet)
...
<<lib>>
x <- as.double(1:5)
loadcppchunk('testNumericVector',compile=compile,quiet=quiet)
...
loadcppchunk('testCNumericVector',compile=compile,quiet=quiet)
...
loadcppchunk('testDate',compile=compile,quiet=quiet)
.Call('testDate',d1, d2)
...
loadcppchunk('testDataFrame1',compile=compile,quiet=quiet)
...
<<lib>>
  loadcppchunk('testDataFrame2',compile=compile,quiet=quiet)
.Call('testDataFrame2')
@
}
...
loadcppchunk('testFactor',compile=compile,quiet=quiet)
.Call('testFactor',f)
@
}  
...
loadcppchunk('testZooSeries1',compile=compile,quiet=quiet)
...
<<lib>>
  loadcppchunk('testZooSeries2',compile=compile,quiet=quiet)
...
an exception it is likely the R will crash (this is a remote possibly
...
<<lib>>
  loadcppchunk('testSaferHello',compile=compile,quiet=quiet)
...
The important message from this section is that
all \C++ code that is called from
R should be bracketed between {\tt BEGIN\_RCPP} and {\tt END\_RCPP}
macros as in this example, and the enclosed \C++ code should not call
R's {\tt Rf\_error()} function. In particular, this means that we should
\emph{not} return objects that have an {\tt operator SEXP()} directly; they
should first be assigned to a {\tt SEXP} variable as in the example, and
this variable should be returned.
...
initialized objects. The author knows of now environments where the
...
\item[{\tt Rcpp::wrap()}] When a \C++ class has the type conversion 
  ...
  the cast to {\tt SEXP} is never needed.
  \item[{\tt check}] Normally {\tt R CMD check} is run on package archives
    that include processed vignettes, that is, the PDF files have already
    been generated. The vignettes are processed during {\tt R CMD build}.
    But it can happen that a package archive with vignettes is missing the
    PDF files. In this case {\tt check} will try to generate the missing
    PDF files by running {\tt pdflatex}. This will not work with \S++ because
    \verb+\srcinclude+ uses a relative path to find package source files
    in {\tt src}, and this relative path is invalid at {\tt check} time.
    Thus be sure to run {\tt R CMD build} before {\tt R CMD check}
    for packages that use \S++.
