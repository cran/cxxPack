...
at the end to get the cross references right).

Note that when using this method to process a vignette the
log file {\tt inst/doc/cpp/compile.log} is not deleted as it
is when building a package for distribution.
